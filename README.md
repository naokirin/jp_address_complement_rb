...
郵便番号データの町域（`town` / `kana_town`）には、「大通西（１〜１９丁目）」「常盤（その他）」「以下に掲載がない場合」のように、通常の住所表記には含まれない括弧付きの情報などが含まれる場合があります。フォームの候補表示やラベル用に「住所としてそのまま使える町域名」だけ欲しいときは、`normalized_town` と `normalized_kana_town` を使ってください。
